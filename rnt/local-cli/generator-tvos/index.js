...
module.exports = yeoman.generators.NamedBase.extend({
  writing: function() {
    ...
    // SomeApp/tvos/SomeApp
    this.fs.copyTpl(
      this.templatePath(path.join('app', '**')),
      this.destinationPath(path.join('tvos', this.name)),
      templateVars
    );

    // SomeApp/tvos/SomeAppTests
    this.fs.copyTpl(
      this.templatePath(path.join('tests', 'Tests.m')),
      this.destinationPath(path.join('tvos', this.name + 'Tests', this.name + 'Tests.m')),
      templateVars
    );
    this.fs.copy(
    ...
    var projectPath = path.resolve(this.destinationRoot(), 'tvos', this.name);
    this.log(chalk.white.bold('To run your app on tvOS:'));
